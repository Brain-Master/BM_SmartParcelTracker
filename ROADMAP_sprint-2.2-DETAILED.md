...
| **Сортировка (частично)** | В Dashboard: dropdown выбора поля сортировки + кнопка направления (↑/↓), сортировка на клиенте по `orderRows` и `orphanParcels`. |

### Не сделано (объём спринта)

1. **2.2.3 Архивирование** — поле `is_archived`, фильтр по умолчанию, переключатель «Показать архив», кнопка «В архив».
2. **2.2.3 Сортировка по колонке** — клик по заголовку таблицы с индикатором ▲/▼ (сейчас только dropdown).
3. **2.2.4 Breakdown в Dashboard** — в развёрнутой карточке заказа показывать: товары / доставка / пошлина / итого.
4. **2.2.2 Четыре вида Dashboard** — переключатель [Заказы] [Посылки] [Товары] [Статус] и четыре разных представления данных.
5. **2.2.7 Статусы заказа/позиций и синхронизация с посылками** — полный набор статусов позиции заказа (в т.ч. «Потеряно»), отображение в UI, автообновление статусов позиций при изменении статуса посылки.

**Важно:** В репозитории миграция 006 уже занята под `deleted_at`. Для архивирования нужна **новая миграция** (например `012_add_is_archived.py`).
...
- [ ] Миграция 012 применена, в БД есть колонки в `orders` и `parcels`.
- [ ] GET `/api/orders/` и GET `/api/parcels/` по умолчанию не возвращают архивированные.
- [ ] С параметром `?include_archived=true` архивированные возвращаются.
- [ ] PUT `/api/orders/{id}` и PUT `/api/parcels/{id}` с `{"is_archived": true}` переводят в архив.
- [ ] В ответах Read присутствует поле `is_archived`.
...
- [ ] По умолчанию архивированные заказы и посылки не показываются.
- [ ] Переключатель «Показать архив» есть; при включении список перезапрашивается и показываются архивированные.
- [ ] Кнопка «В архив» у заказа и посылки; по нажатию запись архивируется и список обновляется.
- [ ] В типах и ответах API используется поле `is_archived`.
...
- [ ] В развёрнутой карточке заказа видна разбивка: товары / доставка / пошлина / итого.
- [ ] Числа соответствуют данным заказа (и при необходимости order_items).
...
- [ ] Переключатель из 4 видов виден и переключает режим.
- [ ] Вид «По заказам» совпадает с текущим поведением.
- [ ] Вид «По посылкам» показывает посылки с вложенными товарами.
- [ ] Вид «По товарам» показывает плоский список товаров с заказом и посылками.
- [ ] Вид «По статусу» группирует по статусу доставки (заказы или посылки).
...
- [ ] Пользователь может архивировать заказы и посылки; по умолчанию они скрыты; переключатель «Показать архив» работает.
- [ ] Сортировка доступна по клику (кнопки полей или заголовки) с индикатором направления.
- [ ] В развёрнутой карточке заказа отображается разбивка стоимости: товары + доставка + пошлина = итого.
- [ ] В Dashboard доступны 4 вида: Заказы, Посылки, Товары, Статус.
...
- [ ] Все новые и затронутые сценарии покрыты тестами (pytest); 0 ошибок ruff и eslint.
