...
| **2.2.2 Три вида Dashboard** | Переключатель [Заказы] [Посылки] [Товары]; вид «По посылкам» с вложенными товарами; вид «По товарам» — плоский список; **группировка во всех видах** (отдельный вид «По статусу» отменён). |

### Не сделано (объём спринта)

1. **2.2.7 Статусы заказа/позиций и синхронизация с посылками** — полный набор статусов позиции заказа (в т.ч. «Потеряно»), отображение в UI, автообновление статусов позиций при изменении статуса посылки.
...
graph LR
  ...
  D --> E[2.2.2 Три view + группировка]
...
## Задача 2.2.2: Три вида Dashboard с группировкой

> **Решение:** Отдельный вид «По статусу» не реализуем. Вместо него во всех трёх видах (Заказы, Посылки, Товары) используется группировка.
...
- [x] Вид «По заказам» совпадает с текущим поведением, с группировкой.
- [x] Вид «По посылкам» показывает посылки с вложенными товарами, с группировкой.
- [x] Вид «По товарам» показывает плоский список товаров с заказом и посылками, с группировкой.
- [x] Группировка во всех видах (вид «По статусу» как отдельный таб отменён).
...
- **Файл:** `frontend/src/pages/DesktopDashboard.tsx`
  ...
  - Группировка по item_status (Ждёт оплаты, В сборке, Отправлено, Получено, Потеряно, Отменено и т.д.) доступна во всех видах Dashboard.
...
- Ручная проверка: архив (переключатель, кнопка «В архив»), сортировка по кнопкам, breakdown в карточке заказа, три вида Dashboard с группировкой; статусы позиций (в т.ч. Потеряно, Заказ отменён), смена статуса посылки и отображение обновлённого статуса позиции.
...
- `frontend/src/pages/DesktopDashboard.tsx` — showArchived, переключатель, кнопки «В архив», сортировка по кнопкам (опционально), breakdown стоимости, viewMode и 3 вида с группировкой; отображение статусов позиций (в т.ч. Потеряно)
...
- [x] В Dashboard доступны 3 вида с группировкой: Заказы, Посылки, Товары (вид «По статусу» отменён — группировка во всех видах).
