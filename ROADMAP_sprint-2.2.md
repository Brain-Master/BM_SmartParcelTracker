...
**Статус:** 🚧 В работе (Неделя 1 начата)  
...
- [ ] Пользователь может распределить один товар по нескольким посылкам (разные количества).
- [ ] В Dashboard доступны 4 вида: Заказы, Посылки, Товары, Статус.
- [ ] Работают сортировка по колонкам, удаление с подтверждением, архивирование и переключатель «Показать архив».
- [ ] В заказе отображается разбивка: товары + доставка + пошлина = итого.
...
Неделя 2 — Task 2.2.3 (is_archived, sort, delete/archive) + завершение 2.2.4 (UI).  
...
- [ ] В ParcelForm можно указать несколько товаров и количество в посылке (frontend — след. шаг).
- [ ] В Dashboard видно распределение товара по посылкам (frontend — след. шаг).
...
| Шаг | Файл | Действие |
|-----|------|----------|
| 1 | `backend/alembic/versions/006_add_is_archived.py` | ADD COLUMN is_archived BOOLEAN DEFAULT FALSE в orders и parcels. |
...
- [ ] В списках по умолчанию не показываются архивированные.
- [ ] Есть переключатель «Показать архив» и кнопка «В архив».
- [ ] Удаление с подтверждением, после удаления список обновляется.
- [ ] Сортировка по выбранной колонке с визуальным индикатором.
- [ ] Тесты: list с include_archived, archive order/parcel.
...
- [ ] В OrderForm ввод shipping_cost и customs_cost, отображается итого.
- [ ] В Dashboard отображается полная стоимость заказа (breakdown где уместно).
...
- [ ] Переключатель из 4 видов виден и работает.
...
- [ ] Вид «По статусу» группирует по статусу доставки.
...
- `backend/alembic/versions/006_add_is_archived.py`
