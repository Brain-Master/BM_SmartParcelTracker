...
- [x] В Dashboard доступны 3 вида с группировкой: Заказы, Посылки, Товары (отдельный вид «По статусу» отменён — вместо него группировка во всех видах).
...
Неделя 3–4 — группировка во всех view (вместо отдельного вида «По статусу») + тесты и полировка.
...
**Цель:** Три режима отображения с группировкой во всех видах.

**Views:**

1. **По заказам (By Orders)** — вид по умолчанию (order-centric, expand items), с группировкой.
2. **По посылкам (By Parcels)** — parcel-centric: трек, статус, вес, вложенные товары, с группировкой.
3. **По товарам (By Items)** — плоский список order_items: название, заказ, посылки, статус, с группировкой.

**Решение:** Отдельный вид «По статусу» не реализовывали — вместо него во всех трёх видах добавлена группировка (по статусу и другим полям где уместно).
...
- [x] Группировка реализована во всех видах (отдельный таб «По статусу» отменён).
