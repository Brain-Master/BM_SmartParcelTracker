...
**Следующий фокус:** Завершение 2.2 и приёмка — см. **[ROADMAP_sprint-2.3.md](./ROADMAP_sprint-2.3.md)** (кеш/polling, режим приёмки, синхронизация статусов). Вид «По статусу» отменён: реализована группировка во всех видах Dashboard. Ниже — описание задач Sprint 2.2.
...
4. ~~**По статусу (By Status)**~~ — отменён: вместо отдельного вида реализована **группировка во всех видах** (Заказы, Посылки, Товары).
...
**Backend:**
- ✅ Миграция `012_add_is_archived_to_orders_and_parcels.py`:
  ```sql
...
- [x] Пользователь может разместить один товар в несколько посылок
- [x] Dashboard имеет 3 вида с группировкой (Заказы, Посылки, Товары); вид «По статусу» отменён в пользу группировки во всех видах
- [x] Пользователь может сортировать, удалять и архивировать заказы/посылки
- [x] Стоимость заказа отображается как breakdown (товары + доставка + пошлина)
...
- [x] Все изменения покрыты тестами (pytest для backend)
- [x] 0 linting ошибок (ruff, eslint)
...
**Статус Sprint 2.2:** 🚧 В работе (частично выполнен) — план: [ROADMAP_sprint-2.2.md](./ROADMAP_sprint-2.2.md)
